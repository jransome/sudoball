import {
  ...
  GOAL_DEPTH,
  PITCH_BORDER_COLOUR,
  PITCH_DARK_STRIPE_COLOUR,
  PITCH_LIGHT_STRIPE_COLOUR,
} from './config';
...
import {
  ...
  lowerPitchVertices,
  upperPitchVertices,
  bluePenaltyBoxVertices,
  redPenaltyBoxVertices,
  blueGoalBar,
  redGoalBar,
  penaltyArcCentres,
  postPositions,
...
const paintLine = (ctx: CanvasRenderingContext2D, strokeColour: string, vertices: ReadonlyArray<readonly [number, number]>, pixelsPerMeter: number, lineWidth = 1) => {
  ctx.beginPath();
...
const paintArc = (ctx: CanvasRenderingContext2D, fillColour: string, strokeColour: string, x: number, y: number, radius: number, startAngle: number, endAngle: number, pixelsPerMeter: number, lineWidth = 1) => {
  ctx.beginPath();
...
const paintCircle = (ctx: CanvasRenderingContext2D, fillColour: string, strokeColour: string, x: number, y: number, radius: number, pixelsPerMeter: number, lineWidth = 1) =>
  paintArc(ctx, fillColour, strokeColour, x, y, radius, 0, 360, pixelsPerMeter, lineWidth);
...
const paintGrass = () => {
  ...
  for (let j = 0; j < GAME_ENCLOSURE.x / 2 - 2; j++) {
    let colour;
    if (j % 2 === 0) {
      colour = PITCH_DARK_STRIPE_COLOUR;
    } else {
      colour = PITCH_LIGHT_STRIPE_COLOUR;
    }
    ctx.save();
  ...
  ctx.fillStyle = PITCH_BORDER_COLOUR;
...
const paintPitch = (ctx: CanvasRenderingContext2D) => {
  ...
  paintLine(ctx, 'white', redGoalBar, PIXELS_PER_METER, 2);
  paintLine(ctx, 'white', postPositions.slice(-2), PIXELS_PER_METER); // blue goal line
  paintLine(ctx, 'white', blueGoalBar, PIXELS_PER_METER, 2);
...
const paint = (gameState: TransmittedGameState, localPlayerId: PeerId) => {
  ...
  paintPitch(ctx);
  paintCircle(ctx, 'white', 'black', ...gameState.slice(0, 2) as [number, number], BALL_RADIUS, PIXELS_PER_METER, 1.5); // ball
